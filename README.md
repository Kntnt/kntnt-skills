...
- **`/fabulous`** – toggle the fabulous working mode: the agent orchestrates — thinks, plans, briefs, verifies — while subagents execute on the lowest model judged able to do the job (haiku, sonnet, opus or fable — never one stronger than the delegating agent itself), except when doing the job itself is cheaper. The mode governs only choices you leave open — an explicit "do this yourself" or "spawn a subagent" always wins. Turning it on notes your current model and effort; turning it off reports and clears what was noted. Switching to Fable at high effort — and back — is yours to do with `/model` and `/effort`; the skill only confirms its state.
...
Run `/fabulous` to toggle the fabulous working mode, `/fabulous --on` / `--off` to force a state, and `/fabulous --status` to see the current state and the noted model/effort. While on, the agent works as an orchestrator: it does the thinking — understanding, planning, deciding, briefing, verifying — and delegates execution to subagents on the lowest model judged able to do the job at the effort level the subagent inherits — choosing along the ladder haiku < sonnet < opus < fable, never a model stronger than the one the agent itself runs — except when doing the job itself costs fewer tokens than the brief, the agent's reading and the report together; when in doubt it delegates. The mode governs only choices you leave open: an explicit instruction naming the executor — "do this yourself", "spawn a subagent", "use haiku" — always wins, on or off.

Turning the mode on notes your current model and effort in `fabulous.json` in the session's scratchpad directory; turning it off reports and clears them. Because the scratchpad is unique per session, the mode is per-session — a fresh session always starts off (though your `/model` and `/effort` choices persist on their own). The skill only ever confirms its state — it never switches model or effort (a skill cannot run interactive commands) and never prompts you to: run `/model fable` and `/effort high` yourself when turning it on, and restore afterwards with the values the off-confirmation (or `--status`) shows. In a very long or compacted session, re-assert the standing instruction with `/fabulous --on` (it never overwrites the noted values).
