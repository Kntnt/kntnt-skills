...
2. **Structural sync checks — inferred aggressively.** Whenever the plugin keeps a hand-maintained index that must mirror files on disk (a `lib/.../_index.md` ↔ its sibling files, a `CANONICAL_ORDER` ↔ its modules, the `${CLAUDE_PLUGIN_ROOT}/lib/…` paths a skill reads ↔ the files on disk), generate the symmetry check both directions — unless the reverse would fire on healthy files, as it does for the `lib/` paths, where a file no path names is reached another way (as a directory, or through prose). These indexes rot silently; a check is cheap insurance.
